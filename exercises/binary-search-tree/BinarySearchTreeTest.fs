...
let ``Complex tree`` () =
    ...
    Assert.That(tree |> value, Is.EqualTo(4))
    Assert.That(tree |> left |> Option.map value, Is.EqualTo(Some 2))
    Assert.That(tree |> left |> Option.bind left |> Option.map value, Is.EqualTo(Some 1))
    Assert.That(tree |> left |> Option.bind right |> Option.map value, Is.EqualTo(Some 3))
    Assert.That(tree |> right |> Option.map value, Is.EqualTo(Some 6))
    Assert.That(tree |> right |> Option.bind left |> Option.map value, Is.EqualTo(Some 5))
    Assert.That(tree |> right |> Option.bind right |> Option.map value, Is.EqualTo(Some 7))
