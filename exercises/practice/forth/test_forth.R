...
test_that("User-defined words - can consist of built-in words", {
  expected <- Some c(1, 1, 1)
  expect_equal(evaluate c(": dup-twice dup dup ;", "1 dup-twice"), expected)
})

test_that("User-defined words - execute in the right order", {
  expected <- Some c(1, 2, 3)
  expect_equal(evaluate c(": countup 1 2 3 ;", "countup"), expected)
})

test_that("User-defined words - can override other user-defined words", {
  expected <- Some c(1, 1, 1)
  expect_equal(evaluate c(": foo dup ;", ": foo dup dup ;", "1 foo"), expected)
})

test_that("User-defined words - can override built-in words", {
  expected <- Some c(1, 1)
  expect_equal(evaluate c(": swap dup ;", "1 swap"), expected)
})
...
test_that("User-defined words - can use different words with the same name", {
  expected <- Some c(5, 6)
  expect_equal(evaluate c(": foo 5 ;", ": bar foo ;", ": foo 6 ;", "bar foo"), expected)
})

test_that("User-defined words - can define word that uses word with the same name", {
  expected <- Some c(11)
  expect_equal(evaluate c(": foo 10 ;", ": foo foo 1 + ;", "foo"), expected)
})
