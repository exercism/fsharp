...
let ``Slices of one from one`` () =
    slices "1" 1 |> should equal ["1"]

let ``Slices of one from two`` () =
    slices "12" 1 |> should equal ["1", "2"]

let ``Slices of two`` () =
    slices "35" 2 |> should equal ["35"]

let ``Slices of two overlap`` () =
    slices "9142" 2 |> should equal ["91", "14", "42"]

let ``Slices can include duplicates`` () =
    slices "777777" 3 |> should equal ["777", "777", "777", "777"]

let ``Slices of a long series`` () =
    slices "918493904243" 5 |> should equal ["91849", "18493", "84939", "49390", "93904", "39042", "90424", "04243"]
