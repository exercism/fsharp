...
let ``One byte`` () =
    decode [0x7fuy] |> should equal [0x7fu]

let ``Two bytes`` () =
    decode [0xc0uy; 0x0uy] |> should equal [0x2000u]

let ``Three bytes`` () =
    decode [0xffuy; 0xffuy; 0x7fuy] |> should equal [0x1fffffu]

let ``Four bytes`` () =
    decode [0x81uy; 0x80uy; 0x80uy; 0x0uy] |> should equal [0x200000u]

let ``Maximum 32-bit integer`` () =
    decode [0x8fuy; 0xffuy; 0xffuy; 0xffuy; 0x7fuy] |> should equal [0xffffffffu]
...
let ``Multiple values`` () =
    decode [0xc0uy; 0x0uy; 0xc8uy; 0xe8uy; 0x56uy; 0xffuy; 0xffuy; 0xffuy; 0x7fuy; 0x0uy; 0xffuy; 0x7fuy; 0x81uy; 0x80uy; 0x0uy] |> should equal [0x2000u; 0x123456u; 0xfffffffu; 0x0u; 0x3fffu; 0x4000u]
