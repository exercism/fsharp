...
test_that("Cleans the number", {
  expected: Result<uint64,string> = Ok 2234567890UL
...
test_that("Invalid when 9 digits", {
  expected: Result<uint64,string> = Error "incorrect number of digits"
  expect_equal(clean("123456789", expected))
})

test_that("Invalid when 11 digits does not start with a 1", {
  expected: Result<uint64,string> = Error "11 digits must start with 1"
