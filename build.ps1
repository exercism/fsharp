...
$nunitConsoleExe = Join-Path $toolsDirectory "nunit.consolerunner/tools/nunit3-console.exe"
...
& $nugetExe install FAKE -Version 4.28.0 -ExcludeVersion -OutputDirectory $toolsDirectory
If (!(Test-Path $fakeExe)) {
	Throw "Could not find fake.exe"
}

& $nugetExe install NUnit -Version 3.2.1  -ExcludeVersion -OutputDirectory $toolsDirectory
If (!(Test-Path $nunitFrameworkDll)) {
	Throw "Could not find nunit.framework.dll"
}

& $nugetExe install NUnit.ConsoleRunner -Version 3.2.1 -ExcludeVersion -OutputDirectory $toolsDirectory
If (!(Test-Path $nunitConsoleExe)) {
	Throw "Could not find nunit3-console.exe"
}

# Use FAKE to execute the build script
& $fakeExe "build.fsx" "default" "-ev" "encoding" "utf-8"